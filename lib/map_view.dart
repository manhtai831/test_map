  ...
  for (final item in jsonData['features'] as List<dynamic>) {
    ...
    for (final coor in (item['geometry']?['coordinates'] as List<dynamic>?)!) {
      ...
      for (final point in (coor as List<dynamic>)) {
        ...
        // if (!isExisted) {
        if (isContain) polygon.points.add(currentPoint);
        // }
      }
...
Future<List<RectMarker>> _mapperPoint(Map<String, dynamic> data) async {
  print('Looopeeeeeeeeeeeeeeeeeee: ${DateTime.now()}');
  final bound = data['bound'];
  final jsonData = data['jsonData'];
  List<RectMarker> pgons = [];
  LatLng? lastPoint;
  bool isNew = true;
  for (final item in jsonData as List<dynamic>) {
    ...
    double rainMount = double.parse(((item as List<dynamic>)[2] * 1.0 as double).toStringAsFixed(4));
    LatLng currentPoint = LatLng(x, y);
    ...
    isNew = true;
    lastPoint = currentPoint;
    RectMarker polygon = RectMarker(
        color:   Colors.blue.withOpacity(double.parse((rainMount / 10).toStringAsFixed(1))),
